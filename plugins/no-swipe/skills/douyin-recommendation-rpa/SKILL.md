...
- Increment user-visible progress only after `status=advanced`, or after `status=stop_required` explicitly returns `stop_phase=next_page_preflight`, `committed=true`, and `transition.ok=true`. The latter means the current item completed and the safety gate belongs to the newly displayed page. Requested batch size, a committed transition-pending row, a successful control call, or a changed local counter does not prove another completed item.
...
- Complete the bounded reread for `status=media_loading` without a chat update. Reconcile `status=transition_pending` with `retryable=true` on the same runner without a chat update. If either state remains unresolved after its permitted recovery and blocks forward progress, stop and give one plain Chinese sentence stating whether the current item was counted, whether its record is safe, and what the user needs to do; keep internal status names out of that message.
...
- Console `React #418/#422` and a `429` **log line** are not safety stops. Stop only when `stop_text_hit` is backed by `stop_evidence.source=visible_blocker` from a viewport-visible dialog, alert, CAPTCHA, verification, rate-limit, restriction, or login-gate container. Matching words in whole-page text, offscreen help copy, stale portals, or recommendation captions are not safety evidence. If the extractor already returns `active_video` without verified stop evidence, continue; do not enter the recovery ladder.
...
- `status=stop_required`: stop immediately for the reported on-page safety signal, then interpret its phase instead of inferring lifecycle from the status alone. `stop_phase=preflight` with `committed=false` means the visible page was not planned or recorded. `stop_phase=post_action` with `committed=true` means the attempted action was recorded but no successful transition was finalized. `stop_phase=next_page_preflight` with `committed=true` and `transition.ok=true` means the current observation and action-driven or delayed transition were finalized successfully; count its returned `progress`, and treat the safety signal as belonging to the next page.
- Preserve the runner result fields `committed`, `stop_phase`, `progress`, `transition`, `record_id`, `upload`, `sync`, and `page` through any batch wrapper or compact projection. Never report whether an item was counted, interacted with, persisted, or uploaded from `status` and `reason` alone.
