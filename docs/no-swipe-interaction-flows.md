...
每个新 Codex 任务都把 `https://www.douyin.com/user/self` 作为首个抖音认号动作；抖音本人页与 No Swipe 工作台使用两个独立标签页，可以并行打开，身份绑定、画像确认和推荐流动作等待 `auth.connected=true`。本人页首次未读出抖音号时再尝试一次，总计两次；登录、验证码、验证页或访问限制直接停止。两次均无阻塞但仍无抖音号时，才从本人页、当前抖音页面、账号菜单或登录账号头像区域读取昵称，并且只接受唯一精确命中，禁用相似昵称匹配。抖音号是唯一裁决：新抖音号在同一 No Swipe 用户下创建并列账号，不改旧画像；同一抖音号改昵称时自动写入最新昵称并保留旧昵称审计。`no-swipe config profile identity` 完成该记录。同一 Codex 任务内的中断、恢复和连续采集复用已核验身份；新任务重新核验，运行中出现登录或账号矛盾则暂停并重新认号。确认运行后再进入推荐流 `https://www.douyin.com/?recommend=1`。内容卡片中的作者头像不属于登录账号认号入口，推荐作者主页始终不进入。
...
| 2026-08-29 | 0.4.2 | 昵称快路径：昵称唯一命中已绑账号即免开主页，提示一句继续；identity.json 存档昵称 |
...
| 2026-09-02 | 0.4.14 候选修复 | 恢复昵称认号阶梯：已绑定昵称唯一命中时直接复用；昵称陌生、缺失或有歧义时打开固定 `/user/self` 获取抖音号；推荐作者主页继续禁止 |
| 2026-09-02 | 0.4.16 候选 | 每个新 Codex 任务优先打开 `/user/self`，最多尝试两次；无阻塞时才按唯一精确昵称降级。同任务恢复复用身份，新抖音号建立并列账号，同号改昵称自动更新并保留历史 |
