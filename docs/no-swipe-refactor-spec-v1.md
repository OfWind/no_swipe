...
- 产品默认配置可以定义存储、重试和停止策略，但不得定义默认兴趣画像。
...
Skill 收到“开始刷推荐流”“采集 N 条”等请求后，自动加载账号画像，并在操作页面前收齐：
...
3. **明确授权**
   - 点赞、收藏、评论、关注、“不感兴趣”、主页抽样分别确认。
   - 评论启用时，确认语气、禁区和 `per_run` / `per_item` 审批方式。
4. **运行与交付**
   - 是否允许离线缓存后上传。
   - 输出目录、是否生成 CSV / Excel、数据保留要求。

用户可以用“全部不互动”一次性把所有互动率明确设置为 `0`；这不是跳过问询。

Skill 在问询前可以展示一行账号画像摘要并提供“沿用 / 修改”入口；用户选择沿用时不得重复要求输入画像详情。
...
- 用户明确确认后写入 `confirmed_at`、`confirmed_by` 与 `config_hash`。
