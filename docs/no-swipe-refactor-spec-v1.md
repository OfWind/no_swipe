...
- 浏览器授权后的第一步保持在当前抖音推荐流或现有页面，只从页面框架、账号菜单或登录账号头像区域读取可见昵称和抖音号；内容卡片中的作者头像不属于认号入口，不得打开自己或作者的创作者主页。
...
- 可执行互动权限默认全部开启；动作仍必须满足正比例、合格池和总上限。评论权限可为开启而评论率为 0，此时不会评论。旧 contract 中的 `profile_visit` / `profile_sampling` 不授权主页跳转。
...
- 推荐流需要粉丝数、近期作品稳定性或新发布时间证据时，只采用当前推荐卡片可见事实；缺失字段设为 `null` 并保持在推荐流。进入作者主页补证据的旧行为自 0.3.8 起废弃。
...
Skill 收到“开始刷推荐流”“采集 N 条”等请求后，先在当前抖音页面从页面框架、账号菜单或登录账号头像区域识别账号并加载账号画像，再展示一张预设卡。内容卡片中的作者头像不用于认号；身份不可靠时保持未启动，不得切到自己或作者的创作者主页。预设内部必须完整包含：
...
3. **授权**
   - 预设默认开启点赞、收藏、评论、关注和“不感兴趣”等可执行互动权限；主页访问不属于可执行授权，旧 `profile_visit` / `profile_sampling` 字段保持无导航语义。
